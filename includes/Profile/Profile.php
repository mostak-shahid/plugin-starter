<?php
namespace MosPress\PluginStarter\Profile;
...
class Profile
{
	...
	public function profile_scripts($hook)
	{
        ...
        global $user_id;
		wp_enqueue_script(
			'plugin-starter-profile-react',
			PLUGIN_STARTER_URL . 'build/profile.js',
			[],
        ...
        wp_localize_script('plugin-starter-profile-react', 'plugin_starter_profile_obj', $ajax_params);

	}
	public static function display_profile_settings($user) {
        wp_nonce_field( 'plugin_starter_profile_action', 'plugin_starter_profile_field' );
        // echo  $user->ID; 
        echo '<div id="plugin-starter-profile-react-app" class="build-with-bootstrap"></div>';
    }
    ...
    public function get_user_meta($request){
        ...
        return new WP_REST_Response(
            array(
                'success'       => false,
                'user_id'       => $user_id,
                'message'          => __('Invalid User ID', 'plugin-starter'),
            ),
            200
        );
    }
