...
function get_custom_post_type_archive_template(  ) {
    ...
    if ( is_post_type_archive( $custom_post_type ) ) {
        ...
        $theme_files = array('archive-' . $custom_post_type . '.php', 'text-domain' . '/archive-' . $custom_post_type . '.php');
...
function locate_template( $template, $settings, $page_type ) {
    ...
    if ( $exists_in_theme != '' ) {
        ...
        $locations = array(
            join( DIRECTORY_SEPARATOR, array( WP_PLUGIN_DIR, 'text-domain', '' ) ),
            join( DIRECTORY_SEPARATOR, array( WP_PLUGIN_DIR, 'text-domain', $settings['templates_dir'], '' ) ), //plugin $settings['templates'] folder
        );
